...
def detectar_ffmpeg():
    ...
    dir_os = 'win' if sys.platform.startswith('win') else 'linux' if sys.platform.startswith('linux') else 'generic'
...
def escribir_nota_markdown(directorio, titulo, video_id, canal, fecha, duracion_str, duracion_sec, url_origen, capitulos, descripcion_cruda, transcripcion=None):
...
""")
            ...
            if capitulos:
                ...
                for cap in capitulos:
                    ...
                    f.write(f"- [{t_str}]({url_origen}&t={sec_inicio}s) ➔ {cap.get('title', 'Segmento')}\n")
                f.write("\n")

            f.write("## 📝 Descripción Original del Video\n---\n")
            f.write(desc_limpia.strip() if desc_limpia.strip() else "*Sin descripción.*")
...
class YoutubeUniversalDownloader(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.http_session.headers.update({"User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:120.0) Gecko/20100101 Firefox/120.0"})
        ...
        self.RUTA_HISTORIAL = os.path.join(os.path.expanduser("~"), ".ytdownloader_historial.json")
    ...
    def crear_interfaz_avanzada(self):
        ...
        self.frame_superior = ctk.CTkFrame(self, fg_color="#18181A", corner_radius=12, border_width=1, border_color="#27272A")
        ...
        self.btn_pegar = ctk.CTkButton(
            self.frame_superior, text="📋 Pegar", font=("Helvetica", 12, "bold"), height=44, width=90,
            fg_color="#27272A", hover_color="#3F3F46", text_color="#FAFAFA", command=self.pegar_del_portapapeles
        )
        self.btn_pegar.grid(row=0, column=1, padx=4, pady=15)

        self.btn_buscar = ctk.CTkButton(
            self.frame_superior, text="Analizar Origen", font=("Helvetica", 13, "bold"), height=44, width=150,
            fg_color="#2563EB", hover_color="#1D4ED8", text_color="#FFFFFF", command=self.validar_y_analizar,
        )
        ...
        self.col_izquierda = ctk.CTkFrame(self.frame_cuerpo, fg_color="#18181A", corner_radius=12, border_width=1, border_color="#27272A")
        self.col_izquierda.grid(row=0, column=0, padx=(0, 8), pady=0, sticky="nsew")
        self.col_izquierda.grid_columnconfigure(0, weight=1)
        self.col_izquierda.grid_rowconfigure(3, weight=1)  # MEJORA: fila del textbox se corrió a 3

        self.frame_thumb_wrap = ctk.CTkFrame(self.col_izquierda, width=320, height=180, fg_color="#09090B", corner_radius=8)
        self.frame_thumb_wrap.grid(row=0, column=0, padx=15, pady=15)
        self.frame_thumb_wrap.pack_propagate(False)
        self.lbl_preview_img = ctk.CTkLabel(self.frame_thumb_wrap, text="A la espera de telemetría...", font=("Helvetica", 12), text_color="#71717A")
        self.lbl_preview_img.pack(expand=True, fill="both")

        self.frame_datos_atomicos = ctk.CTkFrame(self.col_izquierda, fg_color="#27272A", corner_radius=8)
        self.frame_datos_atomicos.grid(row=1, column=0, padx=15, pady=(0, 6), sticky="ew")
        self.frame_datos_atomicos.grid_columnconfigure(1, weight=1)

        for idx, (lbl_text, var_obj) in enumerate(self.meta_vars.items()):
            lbl_tit = ctk.CTkLabel(self.frame_datos_atomicos, text=lbl_text, font=("Helvetica", 11, "bold"), text_color="#A1A1AA", anchor="w")
            lbl_tit.grid(row=idx, column=0, padx=10, pady=5, sticky="w")
            lbl_val = ctk.CTkLabel(self.frame_datos_atomicos, textvariable=var_obj, font=("Helvetica", 11), text_color="#FFFFFF", anchor="w", justify="left")
            lbl_val.grid(row=idx, column=1, padx=5, pady=5, sticky="ew")

        # NUEVO: etiqueta de peso total estimado (para playlists)
        self.lbl_peso_playlist = ctk.CTkLabel(self.col_izquierda, text="📦 Peso total estimado: -", font=("Helvetica", 10), text_color="#71717A", anchor="w")
        self.lbl_peso_playlist.grid(row=2, column=0, padx=18, pady=(0, 6), sticky="w")

        self.txt_resultados = ctk.CTkTextbox(self.col_izquierda, font=("Courier New", 11), fg_color="#09090B", text_color="#22C55E", border_width=1, border_color="#27272A", corner_radius=8)
        ...
        self.col_derecha = ctk.CTkFrame(self.frame_cuerpo, fg_color="#18181A", corner_radius=12, border_width=1, border_color="#27272A")
        self.col_derecha.grid(row=0, column=1, padx=(8, 0), pady=0, sticky="nsew")
        self.col_derecha.grid_columnconfigure(0, weight=1)
        self.col_derecha.grid_rowconfigure(2, weight=1)  # NUEVO: la zona con scroll se expande
        ...
        self.lbl_titulo_ruta = ctk.CTkLabel(self.frame_ruta, text="Bóveda destino / Carpeta de descargas:", font=("Helvetica", 12, "bold"), text_color="#E4E4E7")
        self.lbl_titulo_ruta.grid(row=0, column=0, sticky="w")
        self.path_entry = ctk.CTkEntry(self.frame_ruta, textvariable=self.download_path, state="readonly", fg_color="#09090B", border_color="#27272A", height=36)
        self.path_entry.grid(row=1, column=0, padx=(0, 6), pady=5, sticky="ew")
        self.path_btn = ctk.CTkButton(self.frame_ruta, text="📁", width=42, height=36, fg_color="#27272A", hover_color="#3F3F46", command=self.examinar_carpeta)
        self.path_btn.grid(row=1, column=1, pady=5)

        # NUEVO: badge de origen de datos (Piped / yt-dlp clásico)
        self.lbl_origen_badge = ctk.CTkLabel(self.col_derecha, text="⚪ Origen: -", font=("Helvetica", 11, "bold"), text_color="#71717A", anchor="w")
        ...
        self.scroll_settings = ctk.CTkScrollableFrame(self.col_derecha, fg_color="transparent", height=330)
        self.scroll_settings.grid(row=2, column=0, padx=15, pady=5, sticky="nsew")
        self.scroll_settings.grid_columnconfigure(0, weight=1)

        # Formato y calidad
        self.frame_settings = ctk.CTkFrame(self.scroll_settings, fg_color="#09090B", corner_radius=8, border_width=1, border_color="#27272A")
        self.frame_settings.pack(fill="x", pady=(0, 10))
        self.frame_settings.grid_columnconfigure((1, 3), weight=1)

        ctk.CTkLabel(self.frame_settings, text="Formato:", font=("Helvetica", 11, "bold"), text_color="#A1A1AA").grid(row=0, column=0, padx=10, pady=12, sticky="w")
        self.format_option = ctk.CTkOptionMenu(self.frame_settings, values=["Video (MP4)", "Solo Audio (MP3)"], command=self.gestionar_cambio_formato, height=30, fg_color="#27272A", button_color="#27272A")
        self.format_option.grid(row=0, column=1, padx=5, pady=12, sticky="ew")

        self.res_label = ctk.CTkLabel(self.frame_settings, text="Calidad:", font=("Helvetica", 11, "bold"), text_color="#A1A1AA")
        self.res_label.grid(row=0, column=2, padx=10, pady=12, sticky="w")
        self.res_option = ctk.CTkOptionMenu(self.frame_settings, values=["Máxima Calidad", "1080p", "720p", "480p"], height=30, fg_color="#27272A", button_color="#27272A")
        self.res_option.grid(row=0, column=3, padx=5, pady=12, sticky="ew")

        # NUEVO: recorte de segmento
        self.frame_recorte = ctk.CTkFrame(self.scroll_settings, fg_color="#09090B", corner_radius=8, border_width=1, border_color="#27272A")
        self.frame_recorte.pack(fill="x", pady=(0, 10))
        ctk.CTkLabel(self.frame_recorte, text="✂️ Recortar segmento (opcional)", font=("Helvetica", 12, "bold"), text_color="#E4E4E7").grid(row=0, column=0, columnspan=4, padx=10, pady=(10, 4), sticky="w")
        self.recorte_var = tk.BooleanVar(value=False)
        ctk.CTkCheckBox(self.frame_recorte, text="Activar recorte", variable=self.recorte_var, font=("Helvetica", 11)).grid(row=1, column=0, columnspan=4, padx=10, pady=4, sticky="w")
        ctk.CTkLabel(self.frame_recorte, text="Inicio:", font=("Helvetica", 11), text_color="#A1A1AA").grid(row=2, column=0, padx=(10, 2), pady=(2, 12), sticky="w")
        self.entry_recorte_inicio = ctk.CTkEntry(self.frame_recorte, placeholder_text="mm:ss", width=80, fg_color="#18181A")
        self.entry_recorte_inicio.grid(row=2, column=1, padx=2, pady=(2, 12), sticky="w")
        ctk.CTkLabel(self.frame_recorte, text="Fin:", font=("Helvetica", 11), text_color="#A1A1AA").grid(row=2, column=2, padx=(10, 2), pady=(2, 12), sticky="w")
        self.entry_recorte_fin = ctk.CTkEntry(self.frame_recorte, placeholder_text="mm:ss", width=80, fg_color="#18181A")
        self.entry_recorte_fin.grid(row=2, column=3, padx=2, pady=(2, 12), sticky="w")

        # Automatizaciones Obsidian
        self.frame_obsidian_opt = ctk.CTkFrame(self.scroll_settings, fg_color="#1F1F23", corner_radius=8)
        self.frame_obsidian_opt.pack(fill="x", pady=(0, 10))
        ctk.CTkLabel(self.frame_obsidian_opt, text="Automatizaciones de la Base de Conocimiento", font=("Helvetica", 12, "bold"), text_color="#60A5FA").grid(row=0, column=0, padx=15, pady=(10, 5), sticky="w")

        self.thumb_var = tk.BooleanVar(value=True)
        ctk.CTkCheckBox(self.frame_obsidian_opt, text="Incrustar miniatura HD en contenedor multimedia", variable=self.thumb_var, font=("Helvetica", 11)).grid(row=1, column=0, padx=20, pady=6, sticky="w")

        self.meta_var = tk.BooleanVar(value=True)
        ctk.CTkCheckBox(self.frame_obsidian_opt, text="Forzar metadatos atómicos ID3/XMP (Autor, Enlace, Fecha)", variable=self.meta_var, font=("Helvetica", 11)).grid(row=2, column=0, padx=20, pady=6, sticky="w")

        self.subs_var = tk.BooleanVar(value=False)
        ctk.CTkCheckBox(self.frame_obsidian_opt, text="Extraer pistas de subtítulos nativos (VTT)", variable=self.subs_var, font=("Helvetica", 11)).grid(row=3, column=0, padx=20, pady=6, sticky="w")
        ...
        ctk.CTkLabel(self.frame_transcripcion, text="🎙️ Transcripción automática (Whisper)", font=("Helvetica", 12, "bold"), text_color="#E4E4E7").grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 4), sticky="w")

        self.transcribir_var = tk.BooleanVar(value=False)
        ctk.CTkCheckBox(self.frame_transcripcion, text="Generar transcripción e incluirla en la nota", variable=self.transcribir_var, font=("Helvetica", 11)).grid(row=1, column=0, columnspan=2, padx=10, pady=4, sticky="w")

        self.solo_transcripcion_var = tk.BooleanVar(value=False)
        ctk.CTkCheckBox(self.frame_transcripcion, text="Modo 'solo transcripción' (no conserva video/audio)", variable=self.solo_transcripcion_var, font=("Helvetica", 11)).grid(row=2, column=0, columnspan=2, padx=10, pady=4, sticky="w")

        ctk.CTkLabel(self.frame_transcripcion, text="Modelo:", font=("Helvetica", 11), text_color="#A1A1AA").grid(row=3, column=0, padx=(10, 2), pady=(2, 12), sticky="w")
        self.modelo_whisper_option = ctk.CTkOptionMenu(self.frame_transcripcion, values=["tiny", "base", "small"], height=28, width=100, fg_color="#27272A", button_color="#27272A")
        self.modelo_whisper_option.set("base")
        self.modelo_whisper_option.grid(row=3, column=1, padx=2, pady=(2, 12), sticky="w")
        ctk.CTkLabel(self.frame_transcripcion, text="Requiere 'faster-whisper' u 'openai-whisper' instalado", font=("Helvetica", 9), text_color="#71717A").grid(row=4, column=0, columnspan=2, padx=10, pady=(0, 10), sticky="w")

        # NUEVO: cola de descargas múltiples
        self.frame_cola = ctk.CTkFrame(self.scroll_settings, fg_color="#1F1F23", corner_radius=8)
        self.frame_cola.pack(fill="x", pady=(0, 5))
        ctk.CTkLabel(self.frame_cola, text="📥 Cola de Descargas (varias URLs)", font=("Helvetica", 12, "bold"), text_color="#60A5FA").pack(anchor="w", padx=15, pady=(10, 4))
        ctk.CTkLabel(self.frame_cola, text="Pega una o varias URLs (una por línea):", font=("Helvetica", 10), text_color="#A1A1AA").pack(anchor="w", padx=15)
        self.txt_cola_input = ctk.CTkTextbox(self.frame_cola, height=55, fg_color="#09090B", font=("Helvetica", 10))
        ...
        ctk.CTkButton(frame_cola_botones, text="➕ Añadir", command=self.agregar_a_cola, width=90, height=30, fg_color="#27272A", hover_color="#3F3F46").pack(side="left", padx=(0, 5))
        ctk.CTkButton(frame_cola_botones, text="🗑️ Vaciar", command=self.vaciar_cola, width=90, height=30, fg_color="#27272A", hover_color="#3F3F46").pack(side="left", padx=5)

        self.txt_cola_preview = ctk.CTkTextbox(self.frame_cola, height=65, fg_color="#09090B", font=("Courier New", 10), state="disabled")
        self.txt_cola_preview.pack(fill="x", padx=15, pady=5)

        self.btn_cola_iniciar = ctk.CTkButton(self.frame_cola, text="▶ Descargar Cola (0)", command=self.iniciar_cola_descargas, height=36, fg_color="#2563EB", hover_color="#1D4ED8", text_color="#FFFFFF")
        self.btn_cola_iniciar.pack(fill="x", padx=15, pady=(0, 12))

        # Estado / progreso / acciones (fuera del scroll, siempre visibles)
        self.status_label = ctk.CTkLabel(self.col_derecha, text="Configure los parámetros y pulse el ejecutor térmico.", text_color="#A1A1AA", font=("Helvetica", 12, "bold"), justify="center")
        self.status_label.grid(row=3, column=0, padx=15, pady=(10, 2))

        self.download_progress = ctk.CTkProgressBar(self.col_derecha, height=10, fg_color="#09090B", progress_color="#2563EB")
        ...
        self.frame_botones_accion = ctk.CTkFrame(self.col_derecha, fg_color="transparent")
        self.frame_botones_accion.grid(row=5, column=0, padx=30, pady=(0, 8), sticky="ew")
        self.frame_botones_accion.grid_columnconfigure((0, 1, 2), weight=1)

        self.download_btn = ctk.CTkButton(self.frame_botones_accion, text="Descargar", command=self.iniciar_hilo_descarga, height=48, font=("Helvetica", 13, "bold"), fg_color="#16A34A", hover_color="#15803D", text_color="#FFFFFF")
        self.download_btn.grid(row=0, column=0, padx=3, sticky="ew")

        self.cancel_btn = ctk.CTkButton(self.frame_botones_accion, text="Cancelar", command=self.cancelar_descarga_actual, height=48, font=("Helvetica", 13, "bold"), fg_color="#DC2626", hover_color="#B91C1C", text_color="#FFFFFF", state="disabled")
        self.cancel_btn.grid(row=0, column=1, padx=3, sticky="ew")

        self.btn_abrir_carpeta = ctk.CTkButton(self.frame_botones_accion, text="📂 Abrir", command=self.abrir_ultima_carpeta, height=48, font=("Helvetica", 13, "bold"), fg_color="#27272A", hover_color="#3F3F46", text_color="#FFFFFF", state="disabled")
        self.btn_abrir_carpeta.grid(row=0, column=2, padx=3, sticky="ew")

        self.btn_historial = ctk.CTkButton(self.col_derecha, text="🕘 Ver Historial de Descargas", command=self.mostrar_historial, height=34, fg_color="#27272A", hover_color="#3F3F46", text_color="#FAFAFA", font=("Helvetica", 12))
        ...
        self.btn_tema = ctk.CTkButton(self.frame_prefs, text="🌙 Tema", command=self.alternar_tema, height=30, fg_color="#27272A", hover_color="#3F3F46", font=("Helvetica", 11))
        self.btn_tema.grid(row=0, column=0, padx=2, sticky="ew")

        self.btn_idioma = ctk.CTkButton(self.frame_prefs, text="🌐 ES/EN", command=self.alternar_idioma, height=30, fg_color="#27272A", hover_color="#3F3F46", font=("Helvetica", 11))
        self.btn_idioma.grid(row=0, column=1, padx=2, sticky="ew")

        self.btn_check_update = ctk.CTkButton(self.frame_prefs, text="🔄 yt-dlp", command=lambda: self.verificar_actualizacion_ytdlp(manual=True), height=30, fg_color="#27272A", hover_color="#3F3F46", font=("Helvetica", 11))
        self.btn_check_update.grid(row=0, column=2, padx=2, sticky="ew")

        self.progress_bar = ctk.CTkProgressBar(self, mode="indefinite", height=4, fg_color="#18181A", progress_color="#2563EB")
    ...
    def imprimir_en_tabla(self, texto, limpiar=True, color_error=False):
        ...
        self.txt_resultados.configure(text_color="#EF4444" if color_error else "#10B981")
        if limpiar:
    ...
    def limpiar_interfaz(self):
        ...
        self.update_status_descarga("Configure los parámetros y pulse el ejecutor térmico.", "#A1A1AA")
        self.imprimir_en_tabla("Consola lista para volcado de flujos...")

    def extraer_id_youtube(self, url):
        match = re.search(r'(?:v=|\/v\/|youtu\.be\/|\/embed\/|\/shorts\/|e\/|watch\?v%3D)([^#\&\?]{11})', url)
    ...
    def aplicar_idioma(self):
        ...
        self.btn_cola_iniciar.configure(text=f"{t['descargar_cola']} ({len(self.cola_urls)})")
    ...
    def verificar_actualizacion_ytdlp(self, manual=False):
        ...
        def _check():
            try:
                ...
                res = self.http_session.get("https://api.github.com/repos/yt-dlp/yt-dlp/releases/latest", timeout=5)
                if res.status_code == 200:
                    ultima = (res.json().get("tag_name") or "").lstrip("v")
                    if ultima and ultima != actual:
                        self.after(0, lambda: self.update_status_descarga(f"🔔 Nueva versión de yt-dlp disponible: {ultima} (tienes {actual}). Ejecuta: pip install -U yt-dlp", "#F59E0B"))
                    elif manual:
                        self.after(0, lambda: self.update_status_descarga(f"✅ yt-dlp está actualizado (v{actual}).", "#10B981"))
                elif manual:
                    self.after(0, lambda: self.update_status_descarga("⚠️ No se pudo consultar la última versión de yt-dlp.", "#F59E0B"))
            except Exception:
                if manual:
                    self.after(0, lambda: self.update_status_descarga("⚠️ No se pudo verificar la versión de yt-dlp (¿sin conexión?).", "#F59E0B"))
    ...
    def procesar_transcripcion_y_limpieza(self, ruta_carpeta, info):
        ...
        if self.transcribir_var.get() or self.solo_transcripcion_var.get():
            ...
            if ruta_archivo and os.path.exists(ruta_archivo):
                ...
                self.after(0, lambda: self.update_status_descarga("🎙️ Transcribiendo con Whisper (puede tardar unos minutos)...", "#60A5FA"))
    ...
    def interpretar_error_ytdlp(self, excepcion):
        ...
        reglas = [
            ...
            ("sign in to confirm your age", "🔞 El video tiene restricción de edad y requiere inicio de sesión."),
            ...
            ("video unavailable", "🚫 El video no está disponible (pudo haber sido eliminado)."),
            ("this live event", "🔴 Es una transmisión en vivo que aún no finaliza."),
            ("copyright", "©️ El video fue bloqueado por una reclamación de derechos de autor."),
        ]
        msg_lower = msg.lower()
    ...
    def agregar_al_historial(self, titulo, ruta, tipo="video"):
        entrada = {
            "titulo": titulo, "ruta": ruta, "tipo": tipo,
    ...
    def mostrar_historial(self):
        ...
        if not self.historial_descargas:
            ctk.CTkLabel(marco, text="Aún no hay descargas registradas.", text_color="#71717A").pack(pady=20)
            return
        ...
        for item in reversed(self.historial_descargas):
            ...
            ctk.CTkLabel(fila, text=texto, justify="left", anchor="w", font=("Helvetica", 11)).pack(side="left", fill="x", expand=True, padx=10, pady=8)
            ruta_item = item.get("ruta", "")
            ctk.CTkButton(fila, text="📂", width=36, command=lambda r=ruta_item: self.abrir_carpeta(r)).pack(side="right", padx=8, pady=8)
    ...
    def validar_y_analizar(self):
        ...
        if "list=" in url_cruda:
            ...
            t = threading.Thread(target=self.analizar_playlist_preliminar, args=(url_cruda,), daemon=True)
        ...
        if not video_id:
            self.imprimir_en_tabla("⚠️ Error: ID multimedia no reconocido.", color_error=True)
            return
        ...
        t = threading.Thread(target=self.procesar_analisis_legacy, args=(self.url_limpia,), daemon=True)
        self.registrar_hilo(t)
        t.start()
    ...
    def _peticion_piped_con_reintentos(self, api_url, intentos=2):
        ...
        for i in range(intentos):
            ...
            if i < intentos - 1:
                time.sleep(1)
        raise ultimo_error if ultimo_error else RuntimeError("Fallo desconocido en Piped")
    ...
    def procesar_analisis_piped(self, video_id):
        ...
        try:
            ...
            self.duracion_string_actual = f"{self.duracion_segundos // 60}:{self.duracion_segundos % 60:02d}"
            ...
            for f in info.get("videoStreams", []):
                ...
                if f.get("format") == "MPEG-4" and es_progresivo and not self.stream_directo_piped:
            ...
            if not hubo_streams:
                reporte += " │ Sin flujos de video disponibles.                          │\n"
            reporte += " └───────────────────────────────────────────────────────────┘\n"

            self.resoluciones_detectadas = sorted(resoluciones_set, key=lambda r: int(r.rstrip('p')) if r.rstrip('p').isdigit() else 0, reverse=True)

            if self.esta_cerrando:
                return
            self.after(0, lambda: self.actualizar_ui_metadatos(vistas, likes, self.duracion_string_actual))

            if self.url_miniatura:
                t_thumb = threading.Thread(target=self.descargar_y_renderizar_miniatura, daemon=True)
            ...
            self.after(0, lambda: self.finalizar_fallo_analisis(f"No se pudo analizar el video (yt-dlp ni Piped funcionaron).\nError: {e}"))
        finally:
    ...
    def ejecutar_fallback_piped(self):
        ...
        if not self.id_video_actual:
            ...
            return
        t = threading.Thread(target=self.procesar_analisis_piped, args=(self.id_video_actual,), daemon=True)
        self.registrar_hilo(t)
        t.start()

    def procesar_analisis_legacy(self, url):
        ydl_opts = {"quiet": True, "skip_download": True, "ffmpeg_location": self.ruta_ffmpeg, "http_headers": {"User-Agent": "Mozilla/5.0"}}
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                self.fecha_video_actual = f"{raw_date[6:8]}/{raw_date[4:6]}/{raw_date[0:4]}" if len(raw_date) == 8 else "N/A"
                ...
                self.resoluciones_detectadas = sorted(resoluciones_set, key=lambda r: int(r.rstrip('p')), reverse=True)

                if not self.esta_cerrando:
                    self.after(0, lambda: self.actualizar_ui_metadatos(info.get("view_count"), info.get("like_count"), self.duracion_string_actual))
    ...
    def renderizar_miniatura_en_main(self, img_data):
        try:
            ...
            img_ctk = ctk.CTkImage(light_image=img_res, dark_image=img_res, size=(320, 180))
    ...
    def actualizar_badge_origen(self, origen):
        if origen == "Piped":
            self.lbl_origen_badge.configure(text="🟢 Origen: Piped (rápido)", text_color="#22C55E")
        elif origen == "yt-dlp Clásico":
            self.lbl_origen_badge.configure(text="🟠 Origen: yt-dlp Clásico (respaldo)", text_color="#F59E0B")
        else:
    ...
    def analizar_playlist_preliminar(self, url):
        try:
            ...
            muestra = entries[:3]
            tamanos_muestra = []
            ydl_opts_full = {"quiet": True, "skip_download": True, "ffmpeg_location": self.ruta_ffmpeg, "http_headers": {"User-Agent": "Mozilla/5.0"}}
            for e in muestra:
                try:
                    v_url = e.get("url")
                    if not v_url:
                    ...
                    candidatos = [f.get("filesize") or f.get("filesize_approx") for f in formatos if (f.get("filesize") or f.get("filesize_approx"))]
                    if candidatos:
            ...
            if not self.esta_cerrando:
                self.after(0, lambda: self.finalizar_analisis_playlist(titulo_pl, total, texto_peso))
        except Exception as e:
            if not self.esta_cerrando:
                self.after(0, lambda: self.finalizar_fallo_analisis(f"Error analizando playlist:\n{e}"))
        finally:
            self.limpiar_hilo_terminado(threading.current_thread())
    ...
    def hook_progreso_descarga(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            self.after(0, lambda: self.actualizar_ui_barra(0.95, "Fusing streams & indexando metadatos Obsidian...", "#F59E0B"))
    ...
    def cancelar_descarga_actual(self):
        ...
        self.update_status_descarga("🛑 Cancelando... esperando a que finalice el bloque actual.", "#F59E0B")
        self.cancel_btn.configure(state="disabled")

    def ejecutar_descarga_master(self):
        ruta_base = self.download_path.get()
        if not os.access(ruta_base, os.W_OK):
            self.after(0, lambda: self.finalizar_interfaz_descarga("Error: Sin permisos de escritura.", "#EF4444"))
            return

        ok_espacio, libre_mb = self.verificar_espacio_disco(ruta_base)
        if not ok_espacio:
            self.after(0, lambda: self.finalizar_interfaz_descarga(f"⚠️ Espacio insuficiente en disco ({libre_mb:.0f} MB libres, mínimo 200 MB).", "#EF4444"))
            return
    ...
    def procesar_video_individual(self, ruta_destino):
        ...
        if self.omitir_duplicados_var.get() and self.id_video_actual:
            ...
            if self.id_video_actual in registro:
                self.after(0, lambda: self.finalizar_interfaz_descarga(f"⏭️ Omitido: '{self.titulo_video_actual}' ya se descargó antes en esta carpeta.", "#F59E0B"))
                self.limpiar_hilo_terminado(threading.current_thread())
                return
        ...
        if self.solo_transcripcion_var.get():
            ...
            url = self.stream_directo_piped if self.stream_directo_piped else self.url_limpia
        ...
        try:
            ...
            if self.omitir_duplicados_var.get() and self.id_video_actual:
                registro = self.cargar_registro(ruta_destino)
                registro[self.id_video_actual] = {"titulo": self.titulo_video_actual, "fecha": self.fecha_video_actual}
                self.guardar_registro(ruta_destino, registro)

            self.after(0, lambda: self.finalizar_flujo_individual(ruta_destino, capitulos, desc, transcripcion, integridad_ok))
        except DownloadCancelled:
            self.after(0, lambda: self.finalizar_interfaz_descarga("🛑 Descarga cancelada por el usuario.", "#F59E0B"))
        except DownloadError as e:
            mensaje = self.interpretar_error_ytdlp(e)
            self.after(0, lambda m=mensaje: self.finalizar_interfaz_descarga(m, "#EF4444"))
        except Exception as e:
            self.after(0, lambda: self.finalizar_interfaz_descarga(f"Fallo multimedia: {str(e)[:60]}", "#EF4444"))
        finally:
            self.limpiar_hilo_terminado(threading.current_thread())
    ...
    def finalizar_flujo_individual(self, ruta, capitulos, desc, transcripcion=None, integridad_ok=True):
        ...
        if integridad_ok:
            self.finalizar_interfaz_descarga("Sincronización de recurso completada con éxito.", "#10B981")
        else:
            self.finalizar_interfaz_descarga("⚠️ Descarga completada pero el archivo parece incompleto (revisa tu conexión).", "#F59E0B")
    ...
    def procesar_playlist_lote(self, ruta_base):
        ...
        try:
            ...
            titulo_pl = self.saneamiento_titulo_archivo(playlist_info.get("title", "Playlist_Set"))
...
""")
            ...
            for idx, entry in enumerate(entries):
                ...
                if omitir_duplicados and v_id_flat:
                    ...
                    if v_id_flat in registro:
                        with open(moc_path, "a", encoding="utf-8") as f_moc:
                            f_moc.write(f"- ⏭️ (Omitido, ya descargado antes) {v_id_flat}\n")
                        continue

                self.after(0, lambda i=idx: self.update_status_descarga(f"Lote: {i+1}/{total_videos} ➔ Sincronizando...", "#60A5FA"))
                ...
                try:
                    with yt_dlp.YoutubeDL(ydl_opts_descarga) as ydl:
                        ...
                        v_fec = f"{raw_date[6:8]}/{raw_date[4:6]}/{raw_date[0:4]}" if len(raw_date) == 8 else "N/A"
                    ...
                    with open(moc_path, "a", encoding="utf-8") as f_moc:
                        if integridad_ok_v:
                            f_moc.write(f"- [[{file_saneado}]] | *{v_dur_str}* - {v_can}\n")
                        else:
                            f_moc.write(f"- [[{file_saneado}]] | *{v_dur_str}* - {v_can} ⚠️ (posible descarga incompleta)\n")
                except DownloadCancelled:
                    break
                except DownloadError as e:
                    mensaje = self.interpretar_error_ytdlp(e)
                    ...
                    print(f"Error omitido en elemento de playlist index {idx}: {e}", file=sys.stderr)
                    continue

            self.ultima_ruta_descarga = carpeta_pl
            self.agregar_al_historial(playlist_info.get('title', 'Playlist'), carpeta_pl, tipo="playlist")

            if self.evento_cancelar.is_set():
                self.after(0, lambda: self.finalizar_interfaz_descarga("🛑 Descarga de playlist cancelada por el usuario.", "#F59E0B"))
            else:
                self.after(0, lambda: self.finalizar_interfaz_descarga("Playlist y MOC estructural creados con éxito.", "#10B981"))
        except Exception as e:
            self.after(0, lambda: self.finalizar_interfaz_descarga(f"Fallo en Playlist: {str(e)[:50]}", "#EF4444"))
        finally:
            self.limpiar_hilo_terminado(threading.current_thread())
    ...
    def construir_opciones_ytdlp(self, ruta):
        ydl_opts = {
            ...
            'ffmpeg_location': self.ruta_ffmpeg, 'postprocessors': [],
        ...
        if not self.solo_transcripcion_var.get():
            ...
            if self.subs_var.get():
                ydl_opts.update({'writesubtitles': True, 'subtitleslangs': ['es', 'en']})
    ...
    def corregir_a_markdown(self, directorio, titulo, video_id, canal, fecha, duracion_str, duracion_sec, url_origen, capitulos, descripcion_cruda, transcripcion=None):
        escribir_nota_markdown(directorio, titulo, video_id, canal, fecha, duracion_str, duracion_sec, url_origen, capitulos, descripcion_cruda, transcripcion)
    ...
    def agregar_a_cola(self):
        ...
        agregadas = 0
        for linea in lineas:
            if ("youtube.com" in linea or "youtu.be" in linea) and linea not in self.cola_urls:
                self.cola_urls.append(linea)
                agregadas += 1
        self.txt_cola_input.delete("1.0", "end")
        self.actualizar_preview_cola()
        self.update_status_descarga(f"➕ {agregadas} URL(s) añadida(s) a la cola.", "#60A5FA")
    ...
    def actualizar_preview_cola(self):
        ...
        self.btn_cola_iniciar.configure(text=f"▶ Descargar Cola ({len(self.cola_urls)})")
    ...
    def iniciar_cola_descargas(self):
        ...
        ruta_base = self.download_path.get()
        if not os.access(ruta_base, os.W_OK):
            self.update_status_descarga("Error: Sin permisos de escritura en la carpeta destino.", "#EF4444")
            return
        ok_espacio, libre_mb = self.verificar_espacio_disco(ruta_base)
        if not ok_espacio:
            self.update_status_descarga(f"⚠️ Espacio insuficiente en disco ({libre_mb:.0f} MB libres).", "#EF4444")
            return
        ...
        t = threading.Thread(target=self._procesar_cola_worker, args=(ruta_base,), daemon=True)
        self.registrar_hilo(t)
        t.start()
    ...
    def _descargar_video_individual_directo(self, url, ruta_base):
        ...
        ydl_opts_meta = {"quiet": True, "skip_download": True, "ffmpeg_location": self.ruta_ffmpeg, "http_headers": {"User-Agent": "Mozilla/5.0"}}
        with yt_dlp.YoutubeDL(ydl_opts_meta) as ydl:
        ...
        if self.omitir_duplicados_var.get() and video_id:
            ...
            if video_id in registro:
                self.after(0, lambda: self.imprimir_en_tabla(f"⏭️ Omitido (ya descargado): {titulo}\n", limpiar=False))
                return
        ...
        try:
            ...
            mensaje = self.interpretar_error_ytdlp(e)
            self.after(0, lambda: self.imprimir_en_tabla(f"❌ {titulo}: {mensaje}\n", limpiar=False))
            return
        ...
        raw_date = info.get("upload_date", "") or ""
        fecha = f"{raw_date[6:8]}/{raw_date[4:6]}/{raw_date[0:4]}" if len(raw_date) == 8 else "N/A"
    ...
    def _procesar_cola_worker(self, ruta_base):
        ...
        for idx, url in enumerate(urls_pendientes):
            if self.evento_cancelar.is_set():
                break
            self.after(0, lambda i=idx: self.update_status_descarga(f"Cola: {i+1}/{total} ➔ procesando...", "#60A5FA"))
            try:
                if "list=" in url:
                    self.url_limpia = url
                    self.procesar_playlist_lote(ruta_base)
    ...
    def finalizar_cola(self, exitosos, total):
        ...
        if cancelado:
            texto = f"🛑 Cola cancelada. {exitosos}/{total} completados antes de cancelar."
            color = "#F59E0B"
        else:
...
def ejecutar_modo_cli():
    ...
    parser.add_argument("-o", "--output", default=os.path.join(os.path.expanduser("~"), "Downloads"), help="Carpeta de destino")
    parser.add_argument("--audio", action="store_true", help="Descargar solo audio (MP3)")
    parser.add_argument("--quality", default="best", help="Altura máxima de video, ej. 1080, 720, 480 (ignorado con --audio)")
    parser.add_argument("--transcribe", action="store_true", help="Generar transcripción con Whisper e incluirla en la nota")
    parser.add_argument("--transcribe-only", action="store_true", help="Solo transcribir: descarga audio ligero, transcribe y borra el audio")
    parser.add_argument("--whisper-model", default="base", choices=["tiny", "base", "small"], help="Modelo de Whisper a usar")
    ...
    if args.transcribe_only or args.audio:
        ...
        formato = f"bestvideo[height<={args.quality}]+bestaudio/best[height<={args.quality}]"
    else:
    ...
    ydl_opts = {
        ...
        'format': formato,
        'quiet': False, 'no_warnings': True, 'retries': 5,
    ...
    for entry in entries:
        ...
        url_final = f"https://www.youtube.com/watch?v={video_id}" if video_id else args.url
        ...
        ruta_nota = escribir_nota_markdown(args.output, titulo, video_id, canal, fecha, dur_str, dur_sec, url_final, capitulos, desc, transcripcion)
